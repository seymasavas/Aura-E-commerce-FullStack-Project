...
export default function ProductOverview() {
  ...
  //   12: productcard12,
  // };
  console.log("1. URL'den Gelen Parametreler:", params);
  console.log("2. Redux'taki Ürün Listesi:", productList);

  const currentProduct = productList.find((product) => product.id == productId);

  const [activeImageIndex, setActiveImageIndex] = useState(0);

  console.log("3. Eşleşen Ürün Bulundu mu?:", currentProduct);
  ...
  }, [productId, dispatch, productList.length]);
  ...
  const addToCart = (productToAdd) => {
    ...
    if (existingProduct) {
      ...
      const updatedCart = [...cart, { count: 1, product: productToAdd }];
      dispatch(setCart(updatedCart));
    }
  };
        ...
        <div className="flex flex-col lg:flex-row gap-[30px] justify-center max-w-[1050px] mx-auto">
          <div className="w-full lg:w-[510px]">
            <div className="relative w-full h-[277px] lg:h-[450px] rounded-[5px] overflow-hidden mb-[20px]">
              ...
              <button
                onClick={handlePrev}
                className="absolute left-[20px] top-1/2 -translate-y-1/2 text-white"
              >
                <ChevronLeft size={60} strokeWidth={2} />
              </button>
              <button
                onClick={handleNext}
                className="absolute right-[20px] top-1/2 -translate-y-1/2 text-white"
              >
          ...
          <div className="w-full lg:w-[510px] pt-[10px] px-[24px] lg:px-0 pb-[30px]">
            ...
            <button
              ...
              className="w-full mt-3 py-2 bg-[#23A6F0] text-white font-bold rounded hover:bg-blue-600 transition-colors"
            >
              Add to Cart
            </button>
            {/*
            <div className="flex items-center gap-[10px]">
              ...
              <button className="w-[40px] h-[40px] flex items-center justify-center bg-white border border-[#E8E8E8] rounded-full hover:bg-gray-100 transition-colors group">
                <Heart
                  size={20}
                  className="text-[#252B42] group-hover:text-red-500 transition-colors"
                />
              </button>
              <button className="w-[40px] h-[40px] flex items-center justify-center bg-white border border-[#E8E8E8] rounded-full hover:bg-gray-100 transition-colors group">
                <ShoppingCart
                  size={20}
                  className="text-[#252B42] group-hover:text-[#23A6F0] transition-colors"
                />
              </button>
              <button className="w-[40px] h-[40px] flex items-center justify-center bg-white border border-[#E8E8E8] rounded-full hover:bg-gray-100 transition-colors group">
                <Eye
